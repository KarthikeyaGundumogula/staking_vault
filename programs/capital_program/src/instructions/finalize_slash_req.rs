...
pub struct FinalizeSlashReq<'info> {
    ...
    #[account(
        ...
        address = vault.locking_token_mint @ SlashRequestError::InvalidLockingMint
    )]
...
impl<'info> FinalizeSlashReq<'info> {
    ...
    fn process_withdrawal(&mut self, amount: u64) -> Result<()> {
        ...
        let new_total_capital = self
            ...
            .checked_sub(amount)
            .ok_or(SlashRequestError::ArithmeticUnderflow)?;
        
        require_gte!(new_total_capital,self.vault.min_cap,SlashRequestError::VaultReachedMinCap);
       ...
       let node_operator_key = self.vault.node_operator.key();
        ...
        let transfer_accounts = TransferChecked {
            ...
            to: self.agent_ata.to_account_info(),
